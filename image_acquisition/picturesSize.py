...
df = pd.DataFrame()
for planet in tqdm(os.listdir()): # 遍历每个类别
    os.chdir(planet)
    for file in os.listdir(): # 遍历每张图像
        try:
            img = cv2.imread(file)
            df = df._append({'类别':planet, '文件名':file, '图像宽':img.shape[1], '图像高':img.shape[0]}, ignore_index=True)
        except:
            print(os.path.join(planet, file), '读取错误')
    os.chdir('../')
os.chdir('../')
